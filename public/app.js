...
function renderUrns(size){const grid=$('urnGrid');grid.innerHTML='';URNS.forEach(u=>{const available=!size||!!u.prices[size],price=size&&u.prices[size];const b=document.createElement('button');b.type='button';b.className='urn-card '+(state.urnId===u.id?'selected ':'')+(size&&!available?'unavailable':'');b.innerHTML=`<img src="${u.img}" alt="${u.name}"><div class="u-body"><h5>${u.name}</h5><div class="u-price">${size?(available?'$'+price.toLocaleString()+' MXN':'No disponible en este tamaño'):'Selecciona un tamaño'}</div></div>`;localizeImg(b.querySelector('img'));if(available){b.onclick=()=>{state.urnId=u.id;if(size)state.weight=size;renderUrns(size);$('next2').disabled=false;};}grid.appendChild(b);});}
